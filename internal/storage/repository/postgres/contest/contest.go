...
func (p *Postgres) GetProblemset(ctx context.Context, contestID int32) ([]models.Problem, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&problem.Charcode, &problem.ID, &problem.WriterID, &problem.Title, &problem.Statement, &problem.Difficulty, &problem.TimeLimitMS, &problem.MemoryLimitMB, &problem.CreatedAt, &problem.WriterUsername); err != nil {
			return nil, err
		}
